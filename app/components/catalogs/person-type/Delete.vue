...
const onConfirm = async () => {
    try {
        loading.value = true;
        await useFetchRequest(`/api/catalogs/person-type/${props.item.id}`, {
            method: 'DELETE',
        });

        toast.add({
            title: $t('common.success'),
            description: $t('catalogs.personType.messages.deleteSuccess', { name: props.item.name }),
        ...
        toast.add({
            ...
            description: $t('catalogs.personType.errors.deleteFailed', { name: props.item.name }),
